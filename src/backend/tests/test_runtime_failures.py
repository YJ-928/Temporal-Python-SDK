...
class TestRuntimeFailures(unittest.TestCase):
    def test_unwrap_temporal_failure_scenarios(self):
        ...
        app_fail_400 = MockFailureInfo("HTTP status code 400: Detail: Invalid email format")
        fail_400 = MockFailure(
            message="Activity task failed",
            application_failure_info=app_fail_400
        )
        ...
        app_fail_conn = MockFailureInfo("ConnectError: [Errno 111] Connection refused")
        fail_conn = MockFailure(
            message="Activity task failed",
            application_failure_info=app_fail_conn
        )
        msg_conn = unwrap_temporal_failure(fail_conn)
        self.assertEqual(msg_conn, "ConnectError: [Errno 111] Connection refused")

        # 4. Malformed JSON
        app_fail_json = MockFailureInfo("JSONDecodeError: Expecting value: line 1 column 1 (char 0)")
        fail_json = MockFailure(
            message="Activity task failed",
            application_failure_info=app_fail_json
        )
        ...
        # 5. Nested cause unwrap
        nested_cause = MockFailure(
            message="Activity failed",
            application_failure_info=MockFailureInfo("Root cause error")
        )
        top_failure = MockFailure(
